...
app.get("/api/quotes", async (req, res) => {
    try {
        ...
        const quotes = results.map((r, i) => {
            ...
            return { symbol: symbols[i], price: 0, error: "Failed to fetch" };
        });
...
app.get("/api/indices", async (req, res) => {
    try {
        ...
        const results = await Promise.allSettled(
            indexSymbols.map(sym => yahooFinance.quote(sym))
        );
...
app.listen(PORT, () => {
    console.log(`✅ Mustaneer proxy server running on http://localhost:${PORT}`);
});
